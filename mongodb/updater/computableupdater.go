...
func (c *ComputableUpdater[T]) Inc(num T) Updater {
	return New(c, "$inc", num)
}

// Mul finalValue = nowValue * num or finalValue = 0 (if nowValue is Not exist)
func (c *ComputableUpdater[T]) Mul(num T) Updater {
	return New(c, "$mul", num)
}
...
type UnsignedComputableUpdater[T UnsignedComputable] struct {
	*ComputableUpdater[T]
}

// Dec finalValue = T(nowValue - num) or finalValue = T(-value) (if nowValue is Not exist)
func (c *UnsignedComputableUpdater[T]) Dec(num T) Updater {
	return New(c, "$inc", -num)
}
