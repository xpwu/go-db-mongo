...
Modifications:
  ...
  - Field-level `truncate` and `minsize` flags are ignored (v2 made propagation impossible; implement MarshalBSON if you need them)
...
修改内容:
  ...
  - 字段级 `truncate` 和 `minsize` 标记被静默忽略（v2 让传递变得不可能；如有需要请自行实现 MarshalBSON）
...
func (sc *PascalStructCodec) describeStructSlow(
	...
	for i := 0; i < numFields; i++ {
		...
		var stags *tagparser.StructTags
		...
		if useJSONStructTags {
			stags, err = tagparser.ParseJSONStructTags(sf)
		} else {
			stags, err = tagparser.ParseStructTags(sf)
		}
		if err != nil {
			return nil, err
		}
